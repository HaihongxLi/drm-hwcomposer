...
bool VARenderer::startRender(HwcVaLayer* layer,uint32_t format){
  ...
  if ((layer->GetLayerData().pi.transform == kHwcTransform270) ||
    ...
    // ALOGE("lhh I915_FORMAT_MOD_Y_TILED");
  ...
  for (std::map<uint32_t, HwcLayer *>::reverse_iterator a = va_layer_map.rbegin(); a != va_layer_map.rend(); a++,index++) {
    ...
    printf("%s:%d gr_handle_t %p\n", __FUNCTION__,__LINE__, gr_handle_t);
    if ((0 == surface_region_[index].width) || (0 == surface_region_[index].height)) {
      ALOGE(", zorder = %d", a->second->GetZOrder());
      continue;
    }
    ...
    // ALOGE("lhh valoop index %d, va_layer_map.size %zd,%d %d %d %d", index, va_layer_map.size(),
    //   output_region_[index].x, output_region_[index].y,
    //   output_region_[index].width, output_region_[index].height);
  }
