...
class HwcLayer {
 public:
  HwcLayer(HwcDisplay *parent_display = nullptr) : parent_(parent_display){};
...
public:
  HwcVaLayer(HwcDisplay *parent_display);

  void addVaLayerMapData(int zorder, HwcLayer* layer){
    va_z_map.emplace(std::make_pair(zorder, layer));
    // ALOGE("lhh %s %f %f %f %f", __FUNCTION__,
    //   layer->GetLayerData().pi.source_crop.left,
    //   layer->GetLayerData().pi.source_crop.top,
    //   layer->GetLayerData().pi.source_crop.right,
    //   layer->GetLayerData().pi.source_crop.bottom);
  }
